...
let spreadChartInitialized = false;
let corrChartInitialized = false;
...
function updateDashboard(data) {
    ...
    // Create or update price chart (real-time)
    if (data.priceData && data.priceData.times.length > 0) {
        ...
        if (!priceChartInitialized) {
            Plotly.newPlot('priceChart', [priceTrace1, priceTrace2], priceLayout);
            priceChartInitialized = true;
        } else {
            // Use react to efficiently update entire traces
            Plotly.react('priceChart', [priceTrace1, priceTrace2], priceLayout);
        }
    }
    
    // Create or update spread & z-score chart (real-time)
    if (analytics.spread && analytics.spread.length > 0) {
        ...
        if (!spreadChartInitialized) {
            Plotly.newPlot('spreadChart', [spreadTrace, zScoreTrace], spreadLayout);
            spreadChartInitialized = true;
        } else {
            Plotly.react('spreadChart', [spreadTrace, zScoreTrace], spreadLayout);
        }
    }
    
    // Create or update rolling correlation chart (real-time)
    if (analytics.rollingCorrelation && analytics.rollingCorrelation.length > 0) {
        ...
        if (!corrChartInitialized) {
            Plotly.newPlot('correlationChart', [corrTrace], corrLayout);
            corrChartInitialized = true;
        } else {
            Plotly.react('correlationChart', [corrTrace], corrLayout);
        }
    }
}
